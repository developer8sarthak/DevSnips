# Tailwind/Sections — Style Tokens (shared reference)

This file is the canonical design-token reference for every `Tailwind/Sections/<category>/<section>/<style>/` variant. All three styles must remain visually distinct and reuse the tokens below so the library stays cohesive across sections.
